...
raw_data=data[,1:4]
{
  t=600;n_sim=100
  set.seed(1)
  qlag_effects_withCI_part1_ssm_600=simulate.counterfactual_outcome_singlet_withCI(t,tx=c(1,rep(0,10)),y_coeffi_table,y_coeffi_var_table,c_coeffi_table,c_coeffi_var_table,raw_data,n_sim=n_sim,printFlag=T)
  set.seed(1)
  qlag_effects_withCI_part2_ssm_600=simulate.counterfactual_outcome_singlet_withCI(t,tx=c(0,rep(0,10)),y_coeffi_table,y_coeffi_var_table,c_coeffi_table,c_coeffi_var_table,raw_data,n_sim=n_sim,printFlag=T)
  lageffects_CIband_simulated_600=plot_simulatedCI(qlag_effects_withCI_part1_ssm_600-qlag_effects_withCI_part2_ssm_600,probs=c(0.05,0.95),printFlag=F)

  plot(0:10,lageffects_CIband_simulated_600$mean,type="l",ylab="q-lag effect",xlab="# lags",bty="n",cex.axis=2,cex.lab=2,ylim=c(-4,0.3))
  polygon(c(0:10,rev(0:10)),c(lageffects_CIband_simulated_600$upper,rev(lageffects_CIband_simulated_600$lower)),col="grey90",border="grey")
  points(0:10,lageffects_CIband_simulated_600$mean,type="l")
  points(0:10,lageffects_CIband_simulated_600$mean,pch=16)
  abline(h=0,lty=3,lwd=2)
  legend("bottomright",legend=c("estimate", "95% CI"),
         pch = c(NA,15), lty=c(1,NA), col=c("black","grey"),
         bty = "n", # remove the bounder of the legend
